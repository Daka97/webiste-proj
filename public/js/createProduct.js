	...
	productContainer.innerHTML += ` 
		<div class='product-card'>
			<div class='product-image'>
				...
				<button class="card-action-btn delete" onclick="openDeletePopup('${data.id}')"><img src="img/delete.png" alt=""></button>
			</div>
			<div class="product-info">
				<h2 class="product-brand">${data.name}</h2>
				<p class="product-description">${data.shortDes}</p>
...
const deleteItem = (id) => {
	...
	.then(data => {
		if(data == 'success'){
			...
			showAlert("some error occurred while deleting item. Try again");
		}
	})
}